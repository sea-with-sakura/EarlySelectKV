...
import utils.passkey_utils.eval_metrics as eval_metrics
...
def make_passkey_retrieval_target(
    retrieval_target_len,
    retrieval_target_style,
    retrieval_target_template,
    retrieval_target_placeholder,
    retrieval_target_wrapper,
):
    raw_answer, wrapped_answer = make_raw_answer(
        retrieval_target_len=retrieval_target_len,
        retrieval_target_style=retrieval_target_style,
        retrieval_target_wrapper=retrieval_target_wrapper,
    )
...
def make_magic_city_number_retrieval_targets(
    retrieval_target_len,
    retrieval_target_style,
    retrieval_target_template,
    city_placeholder,
    ...
    for _ in range(distraction_num + 1):
        raw_city_question, wrapped_city_question = make_raw_answer(
            retrieval_target_len=1,
            retrieval_target_style="city",
            retrieval_target_wrapper=retrieval_target_wrapper,
            exclusions=raw_questions,
        )
        raw_number_answer, wrapped_number_answer = make_raw_answer(
            retrieval_target_len=retrieval_target_len,
            retrieval_target_style=retrieval_target_style,
            retrieval_target_wrapper=retrieval_target_wrapper,
            exclusions=raw_answers,
        )
    ...
    if os.path.isfile(background_dir):
        ...
        for a_file in file_dirs:
            with open(a_file, "r") as background_text_f:
                background_text = background_text_f.read()
                background_text_len = count_words(background_text)
                background_text_LUT.append(
                    {"background_text": background_text, "background_text_len": background_text_len}
                )
    else:
...
def make_content(depths, retrieval_targets, background_text):
    ...
    retrieval_target_start_positions = [
        int(background_len * depth) for depth in depths
    ]  # [start_pos_1, start_pos_2, ...]

    retrieval_target_start_positions = (
        [0] + retrieval_target_start_positions + [background_len]
    )  # [0, start_pos_1, start_pos_2, ..., background_len]

    retrieval_target_start_n_end_positions = [
        (retrieval_target_start_positions[i], retrieval_target_start_positions[i + 1])
        for i in range(len(retrieval_target_start_positions) - 1)
    ]
    ...
    background_chunks = [
        truncate_string_by_word_count(background_text, word_count_start=i, word_count_end=j)
        for i, j in retrieval_target_start_n_end_positions
    ]
...
def make_full_passkey_retrieval_input(background_len, depth, config):
    ...
    background_text = make_background(
        background_dir=config["eval_params"]["background_dir"],
        background_len=background_len,
        background_filling_style=config["eval_params"]["background_filling_style"],
    )
    ...
    for _ in range(config["eval_params"]["depth_num_iterations"]):
        retrieval_target, raw_answer = make_passkey_retrieval_target(
            retrieval_target_len=config["eval_params"]["retrieval_target_len"],
        ...
        if config["eval_params"]["instruction_position"] == "prefix":
            ...
            logger.error(
                f"Invalid config['eval_params']['instruction_position'] input: {config['eval_params']['instruction_position']}."
            )
            raise ValueError
...
def make_full_magic_city_number_retrieval_input(background_len, depth, config):
    ...
    for _ in range(config["eval_params"]["depth_num_iterations"]):
        retrieval_target, wrapped_question, raw_answer, distraction_retrieval_targets, distraction_raw_answers = (
            make_magic_city_number_retrieval_targets(
                retrieval_target_len=config["eval_params"]["retrieval_target_len"],
                retrieval_target_style=config["eval_params"]["retrieval_target_style"],
                retrieval_target_template=config["eval_params"]["retrieval_target_template"],
        ...
        depths, retrieval_targets = get_magic_city_number_depths_n_targets(
            retrieval_target=retrieval_target,
            retrieval_target_depth=depth,
            distraction_retrieval_targets=distraction_retrieval_targets,
        )
...
def get_magic_city_number_depths_n_targets(retrieval_target, retrieval_target_depth, distraction_retrieval_targets):
    ...
    depths = [
        i * retrieval_target_depth / (n_distraction_targets_before_retrieval_target + 1)
        for i in range(1, n_distraction_targets_before_retrieval_target + 1)
    ]
    depths.append(retrieval_target_depth)
    depths = depths + [
        retrieval_target_depth + i * (1 - retrieval_target_depth) / (n_distraction_targets_after_retrieval_target + 1)
        for i in range(1, n_distraction_targets_after_retrieval_target + 1)
    ]
...
def process_raw_exp_results(raw_exp_results, metrics):
    ...
    for i in raw_exp_results:
        ...
        if i["depth"] not in processed_results[i["background_len"]]:
            if "distraction_allowed_exact_match" in metrics or "distraction_allowed_partial_match" in metrics:
                processed_results[i["background_len"]][i["depth"]] = {
                    "answers": [],
                    "responses": [],
                    "distraction_allowed_answers": [],
                }
            else:
        ...
        if "distraction_allowed_exact_match" in metrics or "distraction_allowed_partial_match" in metrics:
            processed_results[i["background_len"]][i["depth"]]["distraction_allowed_answers"].append(
                i["distraction_allowed_answers"]
            )
        ...
        for depth, depth_v in processed_results[background_len].items():
            ...
            for metric in metrics:
                if metric == "distraction_allowed_exact_match" or metric == "distraction_allowed_partial_match":
                    result_by_metric = sum(
                        eval_metrics.eval_by_metric(depth_v["distraction_allowed_answers"], responses, metric)
                    ) / len(responses)
    ...
    for metric in metrics:
        ...
        for background_len_k, background_len_v in processed_results.items():
            ...
            background_len_wise_results[background_len_k][metric] = sum(
                all_reported_metric_under_certain_background_len
            ) / len(all_reported_metric_under_certain_background_len)
...
def check_if_out_of_context_window(longest_input, model_max_len, tokenizer, out_of_max_len_allowed):
    if hasattr(tokenizer, "tokenize"):
        ...
        longest_input_token_len = len(tokenizer.encode(longest_input, bos=False))

    if longest_input_token_len > model_max_len:
        if not out_of_max_len_allowed:
            logger.error(
                f"Longest input ({longest_input_token_len} tokens) exceeds model_max_len ({model_max_len} tokens) when out_of_max_len_allowed is {out_of_max_len_allowed}."
            )
            raise ValueError
        else:
            logger.error(
                f"Longest input ({longest_input_token_len} tokens) exceeds model_max_len ({model_max_len} tokens), but out_of_max_len_allowed is {out_of_max_len_allowed} so experiment continuous."
            )

    else:
        logger.info(
            f"Longest input ({longest_input_token_len} tokens) is below model_max_len ({model_max_len} tokens)."
        )
