...
def prepare_passkey_retrieval_input(config):
    ...
    background_lens = passkey_utils.get_intermediate_values_within_min_max(
        background_len_min, background_len_max, n_background_lens, desc="background_lens", int_rounding=True
    )
    ...
    for background_len in background_lens:
        for depth in depths:
            if config["eval_params"]["dataset"] == "magic_city_number_retrieval":
                ...
                full_inputs, contents, answers, instructions, retrieval_questions, distraction_allowed_answers = (
                    passkey_utils.make_full_input_for_all_iterations(background_len, depth, config)
                )
                    ...
                    raw_exp_results.append(
                        {
                            ...
                            "distraction_allowed_answers": distraction_allowed_answers[i],
                        }
                    )

            else:
                full_inputs, contents, answers, instructions, retrieval_questions = (
                    passkey_utils.make_full_input_for_all_iterations(background_len, depth, config)
                )
