...
import random
import wonderwords
from constants import read_manifest,write_manifest
import sys
...
def _load_paul_graham_essays() -> str:
    ...
    json_candidates = [
        os.environ.get("RULER_PAUL_GRAHAM_ESSAYS_JSON"),
        script_dir / "json" / "PaulGrahamEssays.json",
        repo_root / "dataset" / "background" / "PaulGrahamEssays.json",
    ]
    for candidate in json_candidates:
        if not candidate:
            continue
        essay_path = Path(candidate)
        if essay_path.is_file():
            with essay_path.open(encoding="utf-8") as f:
                return json.load(f)["text"]

    dir_candidates = [
        os.environ.get("RULER_PAUL_GRAHAM_ESSAYS_DIR"),
        repo_root / "dataset" / "background" / "PaulGrahamEssays",
    ]
    for candidate in dir_candidates:
        if not candidate:
            continue
        essay_dir = Path(candidate)
        if essay_dir.is_dir():
            essay_files = sorted(essay_dir.glob("*.txt"))
            if essay_files:
                return "\n".join(path.read_text(encoding="utf-8") for path in essay_files)

    searched = [str(path) for path in json_candidates + dir_candidates if path]
    raise FileNotFoundError(
        "Paul Graham essay background is missing. Run scripts/dataset_prep/download_ruler.sh, "
        "or set RULER_PAUL_GRAHAM_ESSAYS_JSON/RULER_PAUL_GRAHAM_ESSAYS_DIR. "
        f"Searched: {searched}"
    )
...
if args.type_haystack == 'essay':
    essay = _load_paul_graham_essays()
