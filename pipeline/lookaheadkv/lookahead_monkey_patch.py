...
def install_lookaheadkv_monkey_patch() -> None:
    ...
    def patched_block_forward(
        ...
        runtime = getattr(block_self, "lookaheadkv_runtime", None)
        if runtime is None:
        ...
        hidden_out = mlp_out + x
        runtime.observe_layer_output(
            layer_idx=block_self.attn.block_idx,
            hidden_in=hidden_in,
            hidden_mid=x,
    ...
    def patched_forward(
        ...
        runtime = getattr(attn_self, "lookaheadkv_runtime", None)
        ...
        if selection is not None:
            ...
            if isinstance(selection, dict):
                ...
                selection_mask = selection.get("attention_mask")
...
def uninstall_lookaheadkv_monkey_patch() -> None:
    global _ORIG_BLOCK_FORWARD, _ORIG_ATTN_FORWARD, _ORIG_ATTN_SDPA
...
def attach_lookaheadkv_runtime(model: torch.nn.Module, runtime: object) -> None:
    runtime.attach_model(model)
    for block in model.transformer.h:
        block.lookaheadkv_runtime = runtime
        block.attn.lookaheadkv_runtime = runtime


def detach_lookaheadkv_runtime(model: torch.nn.Module) -> None:
    for block in model.transformer.h:
        block.lookaheadkv_runtime = None
        block.attn.lookaheadkv_runtime = None


def install_lookaheadkv_attention(model: torch.nn.Module, runtime: object) -> None:
    install_lookaheadkv_monkey_patch()
    attach_lookaheadkv_runtime(model, runtime)


def uninstall_lookaheadkv_attention(model: torch.nn.Module) -> None:
    detach_lookaheadkv_runtime(model)
    uninstall_lookaheadkv_monkey_patch()
