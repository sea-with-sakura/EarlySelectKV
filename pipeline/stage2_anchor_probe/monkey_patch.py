...
def install_stage2_anchor_monkey_patch() -> None:
    ...
    def patched_block_forward(
        ...
        runtime = getattr(block_self, "stage2_anchor_runtime", None)
        if runtime is None:
            return _ORIG_BLOCK_FORWARD(block_self, x, cos, sin, mask, input_pos, input_pos_maxp1)

        hidden_in = x
        runtime.observe_layer_input(layer_idx=block_self.attn.block_idx, hidden=hidden_in)
        x_normed = block_self.norm_1(hidden_in)
        ...
        if block_self.config.parallel_residual:
            if not block_self.config.shared_attention_norm:
                x_normed = block_self.norm_2(hidden_in)
            x = attention_output + hidden_in
        else:
            x = attention_output + hidden_in
            x_normed = block_self.norm_2(x)

        runtime.observe_layer_mid(layer_idx=block_self.attn.block_idx, hidden=x)
    ...
    def patched_forward(
        ...
        runtime = getattr(attn_self, "stage2_anchor_runtime", None)
        ...
        if getattr(runtime.cfg, "method", "") == "anchor_quality_stage2":
            runtime.record_anchor_quality_probe(
                layer_idx=attn_self.block_idx,
                q_true=q,
                q_true_raw=q_raw,
                k=k,
            ...
            runtime.record_stage2_probe(
                layer_idx=attn_self.block_idx,
                q_true=q,
                k=k,
                route_indices=selection,
                input_pos=input_pos,
            )
        ...
        if time_steps == 1 and k.size(1) != n_head:
...
def uninstall_stage2_anchor_monkey_patch() -> None:
    global _ORIG_BLOCK_FORWARD, _ORIG_ATTN_FORWARD, _ORIG_ATTN_SDPA
...
def attach_stage2_anchor_runtime(model: torch.nn.Module, runtime: object) -> None:
    for block in model.transformer.h:
        block.stage2_anchor_runtime = runtime
        block.attn.stage2_anchor_runtime = runtime


def detach_stage2_anchor_runtime(model: torch.nn.Module) -> None:
    for block in model.transformer.h:
        block.stage2_anchor_runtime = None
        block.attn.stage2_anchor_runtime = None


def install_stage2_anchor_attention(model: torch.nn.Module, runtime: object) -> None:
    install_stage2_anchor_monkey_patch()
    attach_stage2_anchor_runtime(model, runtime)


def uninstall_stage2_anchor_attention(model: torch.nn.Module) -> None:
    detach_stage2_anchor_runtime(model)
    uninstall_stage2_anchor_monkey_patch()
