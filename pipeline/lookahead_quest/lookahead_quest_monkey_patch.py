...
def install_lookahead_quest_monkey_patch() -> None:
    ...
    def patched_block_forward(
        ...
        runtime = getattr(block_self, "lookahead_quest_runtime", None)
        if runtime is None:
            return _ORIG_BLOCK_FORWARD(block_self, x, cos, sin, mask, input_pos, input_pos_maxp1)

        hidden_in = x
        runtime.observe_layer_input(layer_idx=block_self.attn.block_idx, hidden_in=hidden_in, cos=cos, sin=sin)
        ...
        runtime.observe_layer_mid(layer_idx=block_self.attn.block_idx, hidden_mid=x, cos=cos, sin=sin)
    ...
    def patched_forward(
        ...
        runtime = getattr(attn_self, "lookahead_quest_runtime", None)
...
def uninstall_lookahead_quest_monkey_patch() -> None:
    global _ORIG_BLOCK_FORWARD, _ORIG_ATTN_FORWARD, _ORIG_ATTN_SDPA
...
def attach_lookahead_quest_runtime(model: torch.nn.Module, runtime: object) -> None:
    for block in model.transformer.h:
        block.lookahead_quest_runtime = runtime
        block.attn.lookahead_quest_runtime = runtime


def detach_lookahead_quest_runtime(model: torch.nn.Module) -> None:
    for block in model.transformer.h:
        block.lookahead_quest_runtime = None
        block.attn.lookahead_quest_runtime = None


def install_lookahead_quest_attention(model: torch.nn.Module, runtime: object) -> None:
    install_lookahead_quest_monkey_patch()
    attach_lookahead_quest_runtime(model, runtime)


def uninstall_lookahead_quest_attention(model: torch.nn.Module) -> None:
    detach_lookahead_quest_runtime(model)
    uninstall_lookahead_quest_monkey_patch()
