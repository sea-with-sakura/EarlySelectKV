...
def install_lookahead_loki_monkey_patch() -> None:
    ...
    def patched_block_forward(
        ...
        runtime = getattr(block_self, "lookahead_loki_runtime", None)
        if runtime is None:
            return _ORIG_BLOCK_FORWARD(block_self, x, cos, sin, mask, input_pos, input_pos_maxp1)

        hidden_in = x
        runtime.observe_layer_input(layer_idx=block_self.attn.block_idx, hidden_in=hidden_in)
        ...
        runtime.observe_layer_mid(layer_idx=block_self.attn.block_idx, hidden_mid=x)
    ...
    def patched_forward(
        ...
        runtime = getattr(attn_self, "lookahead_loki_runtime", None)
        ...
        y = runtime.compute_attention_output(
            ...
            rope_interleave=attn_self.config.rope_interleave,
            q_pre=q_pre,
            route_k=route_k,
        )
        if y is not None:
...
def uninstall_lookahead_loki_monkey_patch() -> None:
    global _ORIG_BLOCK_FORWARD, _ORIG_ATTN_FORWARD, _ORIG_ATTN_SDPA
...
def attach_lookahead_loki_runtime(model: torch.nn.Module, runtime: object) -> None:
    runtime.attach_model(model)
    for block in model.transformer.h:
        block.lookahead_loki_runtime = runtime
        block.attn.lookahead_loki_runtime = runtime


def detach_lookahead_loki_runtime(model: torch.nn.Module) -> None:
    for block in model.transformer.h:
        block.lookahead_loki_runtime = None
        block.attn.lookahead_loki_runtime = None


def install_lookahead_loki_attention(model: torch.nn.Module, runtime: object) -> None:
    install_lookahead_loki_monkey_patch()
    attach_lookahead_loki_runtime(model, runtime)


def uninstall_lookahead_loki_attention(model: torch.nn.Module) -> None:
    detach_lookahead_loki_runtime(model)
    uninstall_lookahead_loki_monkey_patch()
