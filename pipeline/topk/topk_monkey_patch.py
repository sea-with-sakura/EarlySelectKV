...
from litgpt.model import CausalSelfAttention, KVCache, apply_rope, do_softcapping
...
def install_topk_monkey_patch() -> None:
    ...
    def patched_forward(
        ...
        runtime = getattr(attn_self, "topk_runtime", None)
        ...
        k, v = attn_self.kv_cache(runtime.cache_position_for_input(input_pos), k, v)
        ...
        is_decode_call = pos.numel() == 1 and int(pos.item()) >= int(runtime.cfg.prompt_len)
        sparse_selection = None
        if is_decode_call:
            sparse_selection = runtime.select_attention_indices(layer_idx=attn_self.block_idx, q=q, k=k)

        if sparse_selection is not None:
            selected_k, selected_v = gather_selected_kv(
                k=k,
                v=v,
                indices=sparse_selection.indices,
...
def uninstall_topk_monkey_patch() -> None:
    global _ORIG_ATTN_FORWARD, _ORIG_ATTN_SDPA
...
def attach_topk_runtime(model: torch.nn.Module, runtime: object) -> None:
    for block in model.transformer.h:
        block.attn.topk_runtime = runtime


def detach_topk_runtime(model: torch.nn.Module) -> None:
    for block in model.transformer.h:
        block.attn.topk_runtime = None


def install_topk_attention(model: torch.nn.Module, runtime: object) -> None:
    install_topk_monkey_patch()
    attach_topk_runtime(model, runtime)


def uninstall_topk_attention(model: torch.nn.Module) -> None:
    detach_topk_runtime(model)
    uninstall_topk_monkey_patch()
